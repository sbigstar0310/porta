...
{% endif -%}
{% set cal = sc.get("calibration", {}) %}
{% if cal.get("calls", 0) >= 20 -%}
- 시스템 자기 평가: 추천 시 밝힌 확신도 평균 {{ "%.0f"|format(cal.avg_confidence * 100) }}% vs 실제 적중률 {{ "%.0f"|format(cal.hit_rate * 100) }}% ({% if cal.overconfidence_gap_pct > 3 %}확신을 다소 과하게 표현하는 경향이 있어 보정 중입니다{% elif cal.overconfidence_gap_pct < -3 %}실제 성적이 표현한 확신보다 좋습니다{% else %}확신 표현이 실제 성적과 잘 맞습니다{% endif %})
{% endif -%}
...
{% set cal = sc.get("calibration", {}) %}
{% if cal.get("calls", 0) >= 20 -%}
- System self-assessment: average stated confidence {{ "%.0f"|format(cal.avg_confidence * 100) }}% vs realized hit rate {{ "%.0f"|format(cal.hit_rate * 100) }}% ({% if cal.overconfidence_gap_pct > 3 %}slightly overconfident — being recalibrated{% elif cal.overconfidence_gap_pct < -3 %}results are better than stated confidence{% else %}well calibrated{% endif %})
{% endif -%}
